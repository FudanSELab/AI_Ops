...
def get_min_data(df: DataFrame):
    # 丢弃全列为NA的数据
    df = preprocessing_set.drop_na_data(df)
    # 丢弃全列值相同的数据
    df = preprocessing_set.drop_all_same_data(df)
    # 丢弃不需要的列
    df = preprocessing_set.select_data(df)
    return df
...
if __name__ == "__main__":
    ...
    df = preprocessing_set.merge_data(df_trace=df_total_0,
                                      df_seq=df_total_1,
                                      df_seq_caller=df_total_2)

    # 丢弃没发生Failure的数据
    df = df.loc[df["y_final_result"] == 1]
    df = df.loc[df["y_issue_ms"] != "Success"]
    ...
    # cv, parm = model.dt(df, "y_final_result")
    # print_best_score(cv, parm, "log.txt")

    df = preprocessing_set.sampling(df, "y_issue_ms")
    df = shuffle(df)
    model.dt_multi_label_single(df, "y_issue_ms")

    # dim = df.pop("y_issue_dim_type")
    # df["y_issue_dim_type"] = dim

    # df, y_multi_label = preprocessing_set.convert_y_multi_label(df, "y_issue_ms")
